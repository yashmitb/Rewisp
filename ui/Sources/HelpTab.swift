...
enum HelpContent {
    ...
    static let faq: [QA] = [
        ...
        .init(q: "What are Promises and how do they show up?",
              a: "Rewisp notices when you commit to something in places you write — Notes, Mail, Slack, Discord (*“I’ll send it Friday”*) — and holds it on **Today → Promises**. You never type them. It reads deadlines the way you write them (*“by EOD”*, *“end of the week”*, *“next week”*), tells a real promise from a *“maybe”* (*“I’ll try to”* won’t count), and ignores quoted replies and signatures — only the line you wrote counts. **Confirming a promise arms its reminder**: on the due day a small pill slides down with the full commitment, once a day until you mark it done. When you later write that you did it (*“emailed Dana the report”*), the slip closes itself. AI chats, editors, and ads can never create promises."),
        .init(q: "What do the colours on Loose threads mean?",
              a: "Age. Each thread shows how many nightly digests it has survived, and the knot shifts from calm indigo through violet and amber to red as it gets older — *“new tonight”* is a note, *“5 nights”* is a decision you keep not making. Rewisp matches a thread across nights by what it's **about**, not its exact wording, since the digest rewrites each one every night. Hover a thread for **Ask** (hands it to the search panel) or **Done** (clears it — and tomorrow's rewording of the same thread stays cleared)."),
        .init(q: "What is the map at the top of Today?",
              a: "Your day, drawn by **meaning** instead of by the clock. Each dot is a place you spent real time — size is how long, colour is the topic — and dots are positioned using the same semantic fingerprints that power search, so related work sits together. The thin line is the order you moved (drag the slider, or press **Replay**); the thick curved lines are how often you bounced between two places, which is the one thing you can't feel while it's happening. **Tap any dot to go back to that moment** — it rebuilds what was on screen and what you were doing either side of it, which is what actually brings a memory back. All computed locally, no AI call."),
        .init(q: "A website is filling my memory with junk — can I stop it?",
              a: "**Settings → Privacy → Sites Rewisp ignores.** This is separate from the app list for a reason: everything you open in a browser shares the browser's name, so ignoring a video site by app would ignore your whole browser. The card shows your busiest sites of the last 30 days, so silencing one is a single tap — paste a full URL if you like, Rewisp trims it. A bare domain like *youtube.com* covers the whole site including subdomains; add a path like *reddit.com/r/all* to ignore just one corner. It matches the page's address, not text on it, so a search result that merely mentions the site is still remembered."),
        .init(q: "An app is filling my memory with junk — can I stop it?",
              a: "Yes, and that's a different switch from the kill list. The kill list is *“don't dare”* — privacy. **Settings → Privacy → Apps Rewisp ignores** is *“don't bother”* — a game or media player whose screens are never worth remembering. The card lists your busiest apps of the last 30 days with counts, so ignoring one is a single tap. Ignored apps are skipped entirely: no capture, no row. It's empty by default."),
    ...
    static let troubleshooting: [QA] = [
        ...
        .init(q: "It’s not capturing anything",
              a: "Click **Allow screen access** on the orange card in the menu bar. Don’t just toggle the switch in System Settings — after an update that row is stale and flipping it does nothing. Rewisp clears it and reopens the right page for you, then starts capturing by itself. Also check you’re not paused (⌘⌥P)."),
